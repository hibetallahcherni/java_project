...
public class Dashboard extends JFrame {
    ...
    private JPanel dashboardHome() {
        ...
        // ── 7 stat cards (3+4 or 4+3 grid — use 4 columns, last row fills left)
        ...
        // Fill 7 cards + 1 filler for the 4-column grid
        for (JPanel card : c) cards.add(card);
        cards.add(createFillerCard()); // 8th slot placeholder
    ...
    /** Filler card for empty grid slot */
    ...
    private void coproForm(Coproprietaire c, int id, JTable table) {
        ...
        if (ok) {
            try {
                ...
                if (!appExists) {
                    // Show informative alert with option to create the apartment first
    ...
    /**
     * Updates all 7 stat-card labels with live data.
     */
    private void refreshDashboard(
        ...
        int occupes = (int) apps.stream()
                .filter(a -> copros.stream().anyMatch(c -> c.getAppartement().getNumero() == a.getNumero()))
                .count();
        statAppCount.setText(String.valueOf(apps.size()));
        statAppSub.setText(apps.isEmpty() ? "Aucun appartement" : occupes + " occupé(s) / " + apps.size());
        statAppTrend.setText(apps.isEmpty() ? "" : occupes == apps.size() ? "▲ Complet" : "◌ " + (apps.size() - occupes) + " vacant(s)");

        // Charges
        statChargeCount.setText(String.valueOf(charges.size()));
        statChargeSub.setText(charges.isEmpty() ? "Aucune charge" : String.format("%.2f DT au total", totalCharges));
        statChargeTrend.setText(charges.isEmpty() ? "" : "▲ " + charges.size() + " enregistrée(s)");

        // Fonds
        statFondsCount.setText(String.valueOf(fonds.size()));
        statFondsSub.setText(fonds.isEmpty() ? "Aucun fonds" : String.format("%.2f DT disponibles", totalFonds));
        statFondsTrend.setText(fonds.isEmpty() ? "" : "● " + fonds.size() + " fonds actifs");

        // Appels
        statAppelCount.setText(String.valueOf(appels.size()));
        statAppelSub.setText(appels.isEmpty() ? "Aucun appel généré" : appels.size() + " appel(s) émis");
        statAppelTrend.setText(appels.isEmpty() ? "" : "▲ Dernier appel généré");

        // Paiements
        statPayCount.setText(String.valueOf(paiements.size()));
        statPaySub.setText(notPaid > 0 ? notPaid + " en attente" : paiements.isEmpty() ? "Aucun paiement" : "Tous réglés ✓");
        statPayTrend.setForeground(notPaid > 0 ? DANGER : SUCCESS);
        statPayTrend.setText(paiements.isEmpty() ? "" : notPaid > 0 ? "▼ " + notPaid + " impayé(s)" : "▲ " + paid + " payé(s)");
        ...
        statTotalTrend.setText(totalCharges > 0 ? String.format("▲ Moy. %.0f DT/charge", charges.isEmpty() ? 0 : totalCharges / charges.size()) : "");
    }
